...
function grabServerTime() {
    var time = null;
    $.ajax({
        url: 'https://jayhomelabbuild.site/api/server-time/',
        async: false,
        dataType: 'text',
...
$(document).ready(function() {
    ...
    function renderCountdown() {
        ...
        $.getJSON('https://jayhomelabbuild.site/api/next-service-time/?time=' + new Date().getTime(), function(json) {
    ...
    function halfHourBefore(periods) {
        ...
        if (server_time.getDay() == end_service_day && server_time.getHours() == (end_service_hours + 2) && server_time.getMinutes() == end_service_minutes && server_time.getSeconds() == 0) {
            ...
            $.ajax({
                url: 'https://jayhomelabbuild.site/api/live-recent-message/?time=' + new Date().getTime(),
                dataType: 'json',
        ...
        if (periods != null) {
            if (periods[4] == '0' && periods[5] <= '15') {
                if (live_streaming == false) {
                    live_streaming = true;
                    $.ajax({
                        url: 'https://jayhomelabbuild.site/api/live-message-details/?intro=true&time=' + new Date().getTime(),
                        dataType: 'json',
                        success: function(response) {
    ...
    $("#send_prayer_email").on('click', function(event) {
        ...
        if (honeypot == '' || honeypot == null) {
            ...
            $.post("https://jayhomelabbuild.site/api/prayer-request/", {
    ...
    $("#send_tech_support_email").on('click', function(event) {
        ...
        $.post("https://jayhomelabbuild.site/api/live-support/", {
    ...
    $("#send_note_email").on('click', function(event) {
        ...
        $.post("https://jayhomelabbuild.site/api/email-notes/", {
    ...
    $("#send_note_email_friend").on('click', function(event) {
        ...
        if (honeypot == '' || honeypot == null) {
            ...
            $.post("https://jayhomelabbuild.site/api/email-friend/", {
