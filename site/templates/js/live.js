...
function grabServerTime() {
    var time = null;
    $.ajax({
        url: 'http://localhost:81/api/server-time/',
        async: false,
        dataType: 'text',
...
$(document).ready(function () {
    ...
    function renderCountdown() {
        ...
        $.getJSON('http://localhost:81/api/next-service-time/?time=' + new Date().getTime(), function (json) {
    ...
    function halfHourBefore(periods) {
        ...
        if (server_time.getDay() == end_service_day && server_time.getHours() == (end_service_hours + 2) && server_time.getMinutes() == end_service_minutes && server_time.getSeconds() == 0) {
            ...
            $.ajax({
                url: 'http://localhost:81/api/live-recent-message/?time=' + new Date().getTime(),
                dataType: 'json',
        ...
        if (periods != null) {
            if (periods[4] == '0' && periods[5] <= '15') {
                if (live_streaming == false) {
                    live_streaming = true;
                    $.ajax({
                        url: 'http://localhost:81/api/live-message-details/?intro=true&time=' + new Date().getTime(),
                        dataType: 'json',
                        success: function (response) {
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        var str = String(e.target);
        if (str.indexOf("bible") > -1) {
            if ($('#chat-tab').find("iframe").length == 0) {
                $('#chat-tab').append('<iframe frameborder="0" allowtransparency="true" src="https://www.bible.com/bible/111/matthew.1.niv" class="bible-iframe"  style="padding:0px;margin:0px;width:100%;min-height: 645px;max-height: 700px;height: 100%;" ></iframe>');
            }
        }
    })
    ...
    $("#send_prayer_email").on('click', function (event) {
        ...
        if (honeypot == '' || honeypot == null) {
            ...
            $.post("http://localhost:81/forms-db/prayer-request/", {
    ...
    $("#send_tech_support_email").on('click', function (event) {
        ...
        $.post("http://localhost:81/forms-db/live-support/", {
    ...
    $("#send_note_email").on('click', function (event) {
        ...
        $.post("http://localhost:81/forms-db/email-notes/", {
    ...
    $("#send_note_email_friend").on('click', function (event) {
        ...
        if (honeypot == '' || honeypot == null) {
            ...
            $.post("http://localhost:81/forms-db/email-friend/", {
