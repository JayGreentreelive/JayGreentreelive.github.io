...
if($q != '') {
    ...
    $input->whitelist('q', $q);
    $matches = $pages->find("parent=/lifegroups/, sort=-date, template=post, title|tags.title|category.title|body%=$q");
    $limit = 6;
    $total = $matches->count();
...
<div class='section ' style='background-color:#efefef;'>
        ...
        <?php if( $resultsFound == false ){ ?>
            <div class="container  serif  pd-b-lg">
                <div class="row ">
                    <div class='col-md-12'>
                        <p>Sorry, we can't find any LIFE Groups for this search term.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container pd-t-md pd-b-lg ">
            <div class="row multi-columns-row ">
                <?php
                $groupsMatches = new PageArray();
                $groupsMatches->import($matches);
                $groupsMatches->filter("start=$start,limit=$limit,  sort=-date");
                $groupsMatches->setLimit($limit);
                $groupsMatches->setTotal($total);
                $groupsMatches->setStart($start);
                $pagination =   $groupsMatches->renderPager(array(
                    'nextItemLabel' => "&raquo;",
                    'numPageLinks' => 5,
                    ...
                    $t = new TemplateFile(wire('config')->paths->templates . 'markup/blog/groups-loop-grid.php');
                    $t->set('groups', $groupsMatches);
                    $out = $t->render();
                    echo $out;
            ?>
...
<div class='container'>
 <div class='row mg-t-lg mg-b-lg'>
     <?php
                                  $searchPage = wire('pages')->get('template=group-search');
                      ...
                      <?php
                        ...
                        if(!$data = $cache->get("group_topics", 29990)) {
                            ...
                            {
                                $blog_count = $pages->count("parent=/lifegroups/,  category=$category, template=post");
                                if( $blog_count  > 0 ){
